...
A major simplification. The plugin had grown into an over-engineered multi-agent and deny-hook architecture that the platform docs say is a weak fit for design and coding work, and the cost of running it had crept up to where Pro users could exhaust a 5-hour quota before finishing discovery. This release strips it back to a lean spine with opt-in depth, moves enforcement from hooks into method, and adopts one honest model and effort policy. It also adds the positive replacements for the deleted grounding hooks: a per-agent effort policy that concentrates premium reasoning on planning and final quality, five workflow wirings for the few high-value moments that need many agents, a spec-driven design layer that grounds implementation in real tokens and components, and a suggest-and-wait `/goal` integration. Agents go from 10 to 8 and skills net from 57 to 53 (the simplification merges down to 51, then the new `design-spec` and `feedback-to-todos` skills bring it to 53); 11 hooks (8 of them hard-deny) and the entire cost-mode system are gone. Phase 2 adds the task-driven iterate flow for products that already exist, a clarify-then-dispatch interaction model, completion routing for plugin-built products, and the `feedback-to-todos` skill.

### Removed

- **The deny-hook stack (8 hard-deny hooks).** Deleted `de-tdd-hook.js`, `de-design-grounding-hook.js`, `de-pre-plan-dialogue-hook.js`, `de-pipeline-completion-hook.js`, `de-drift-audit-hook.js`, `de-fidelity-hook.js`, `de-deliverable-path-hook.js`, and `de-process-recall-hook.sh`, plus `de-plan-copy-hook.js`, `de-safety-hook.js`, and `de-prompt-injection-hook.js` (11 hook files in all). Each deleted hook's goal – grounding, no-drift, TDD discipline, canonical paths – moves into the owning skill or agent body as written method, the way `loom-analysis` and `redesign-spec` already enforce with zero hooks. Per the Claude Code docs: "use the permission system rather than a hook to enforce a hard allow or deny." Destructive-command safety and prompt-injection defense are now covered by Claude Code's auto-mode (destructive-command classifier plus always-on prompt-injection probe).
- **The cost-mode system.** Deleted `assets/cost-modes.json`, `scripts/apply-cost-mode.sh`, and the `/design-engineer:cost-mode` command, along with the Light and Full mode machinery throughout commands, skills, and `CLAUDE.md`. One model policy now lives directly in each component's frontmatter, so there is nothing to rewrite per mode.
- **Autopilot mode.** The plugin no longer has two modes – it runs a single step-by-step flow with the user's approval at every stage. The mode picker, the `mode:` field in the project config, and all mode branching throughout commands, skills, agents, and `CLAUDE.md` are gone. Legacy configs that still carry a `mode:` field are ignored gracefully, never read, never displayed, never branched on.
- **Dead code.** Removed the reserved `deliverables_path` config field and note, leftover `status:` and `last_updated:` references, the statusline phase segment, the `.active-workflow` marker, doubled `.design-engineer-plugin/.design-engineer-plugin/...` path artifacts, and the Direct Access review mode.
- **Two agents.** Deleted `deliverable-writer` (its formatting folded into the skill `*-template.md` files) and `context-analyzer` (its context-grounding folded inline – Plan Mode does that work now). Agents drop from 10 to 8.

### Added

- **Spec-driven design (the new `design-spec` skill).** A structured design spec that grounds implementation in the project's real tokens and components, the positive replacement for the deleted design-grounding, drift, and fidelity hooks. A spec is one `.spec.md` per screen: short prose intent plus per-component fenced `yaml` blocks holding reference-only structure (token references, existing-component references by path, states, variants, responsive, accessibility, and EARS acceptance criteria). The `design-spec` skill reads `ui-design-system`'s `design-system.md` and `ui-references-moodboard`'s `references.md` first, so a spec references only names that already exist. Specs are stored feature-scoped at `.design-engineer-plugin/design/features/<feature-slug>/screens/<screen-slug>.spec.md`, with standalone specs at `.design-engineer-plugin/design/specs/`. Authored as a premium planning step in discovery, consumed verbatim by `frontend-implementer` (it pre-fills its component reuse table from each spec's reuse block), and verified by `design-system-auditor` in a spec-conformance pass that fails on mismatch and flags dangling references. Strictness is graduated: required for consequential UI, optional for trivia, never a blocking gate. The skill ships with `references/spec-format.md`, `references/spec-template.md`, `references/examples.md`, and `references/goal-command.md`.
- **Five workflow wirings for the high-value moments that need many agents.** Each wiring is triggered by a natural-language opt-in in the command body, gated on workflow availability (Claude Code v2.1.154+, paid), and falls back to a single-pass inline run so nothing breaks when workflows are off. The design-exploration wiring (per concept direction at the new-product prototype step) and the spec-authoring wiring (per screen during discovery's feature flow) run Opus agents at xhigh effort, the premium-planning investment. The competitor-analysis wiring (per competitor, or delegated to the bundled `/deep-research`) and the per-page audit wiring in `/design-engineer:review` (per page, designer-feedback capture kept after the run) inherit the lighter default.
- **A suggest-and-wait `/goal` integration.** `/goal` is a real Claude Code built-in (v2.1.139+) that loops turns until a completion condition holds, but it is user-invoked only, so the plugin composes a ready-to-paste `/goal` and stops for the user rather than running it. At UI-build moments with a verifiable end state, `commands/development.md`, `agents/frontend-implementer.md`, and `skills/design-spec/` compose a completion condition from the spec's EARS acceptance criteria plus the standing invariants (at least three Playwright iterations of real user flows, zero hardcoded values, only reused or extended components), present it, and wait. Gated on availability; the flow proceeds normally if `/goal` is unavailable or the user declines.
- **The iterate flow for products that already exist.** A commercial codebase the plugin didn't build, or a plugin-built product that has shipped, now opens a fast, task-driven flow instead of the from-scratch pipeline. The flow leads with one line – "tell me what you want to work on, or pick a starting point" – plus four starting points (act on feedback, redesign a design, explore a concept, audit a design) and the built-in free-form path. No new mode, picker, or state name was added: detection stays as-is, and the existing-project path becomes this lighter flow. Authored once in `commands/launch.md` (`## The iterate flow` and `## Task→dispatch map (single source of truth)`); `commands/launch.md` Step 4 and `skills/meta-setup/SKILL.md` Path B reference that map instead of restating it, collapsing the three drifted routing copies into one source.
- **Clarify-then-dispatch, the core interaction model.** A starting point is a conversational entry, not a dispatch trigger. Selecting one makes the plugin ask for detail in natural language first; only after it understands the task does it read the stored project context and dispatch the right pieces (a skill, a workflow, an agent, the spec layer, or `/goal`, matched to the task). A starting point never auto-spawns an agent or workflow on selection. Most iterate-flow work is a scoped edit to something that already exists, so a free-form prompt plus a reliable scoped-edit loop (restate the exact element, file, and property → locate → edit in place → Playwright-verify → scoped PR) is the workhorse, with the heavier layers brought in only when the change warrants them. Lean in process, powerful in capability.
- **Completion routing for plugin-built products.** When the from-scratch pipeline finishes its last step, `commands/development.md` writes a top-level `status: complete` marker into the project config (additive, idempotent, gated on a plugin-built project; `meta-document` mirrors the fact into the compound-documenter pipeline state). Detection then routes a completed plugin-built project to a new `returning_complete` state that opens the iterate flow rather than looping back to the from-scratch returning path. The branch is mirrored in `skills/meta-setup/scripts/detect-state.sh`, `hooks/de-start-state.sh`, and `commands/launch.md` Step 0. Additive and fail-safe: without the marker, behavior is unchanged, and existing projects never reach the branch.
- **The new `feedback-to-todos` skill.** Generalizes the proven `loom-analysis` method from video-only to four input types – a feedback video, written notes or docs, chat messages, and transcripts – merging multiple sources into one grounded `FEEDBACK.md` of literal, cited to-do items. It keeps loom's discipline verbatim: a "how to work with this file" protocol, literal cited checkbox items, inference stays in chat, a completeness pass, and an "is it already done?" check. Loom's production-vs-prototype rule is generalized to reference-vs-product, so the on-screen UI isn't assumed to be the thing being changed. The bundled `scripts/process_video.sh` replaces loom's silent soft-skip with a proactive preflight: if ffmpeg, whisper, or the transcription model is missing, it prints the exact install command and stops rather than producing an ungrounded, frames-only analysis. The `FEEDBACK.md` protocol feeds the iterate flow's scoped-edit loop one item at a time, and can compose a `/goal` for a verifiable batch as suggest-and-wait. The skill is the iterate flow's "act on feedback" starting point.

### Changed

- **Merged three psychology a/b pairs into one skill each**, keeping their split `references/`: `psych-decision-fundamentals` absorbs `decision-persuasion`, `psych-delight-design` absorbs `emotional-retention`, and `psych-pricing-psychology` absorbs `habit-formation`. Folded three meta-setup stubs (`meta-setup-welcome`, `-existing`, `-configure`) into `meta-setup`. Skills drop from 57 to 51 in the merge, then the new `design-spec` and `feedback-to-todos` skills bring the count to 53.
- **One model and effort policy, grounded in the verified mechanics.** A skill's `model:`/`effort:` frontmatter is inert when the skill is loaded inline, which is how all 53 plugin skills load, so the only runtime levers are the 8 agents' frontmatter and the user's session setting. The policy reflects that: the agents carry an explicit per-agent model and effort, skill frontmatter stays for compliance, and the docs recommend running a strong session model at high effort for the inline planning phases the plugin cannot set itself (MVP, information architecture, Plan Mode). All `claude-opus-4-7` pins and `effort: xhigh` values are gone; components use aliases.
- **Per-agent effort policy.** `design-system-auditor` runs on Opus at high effort, the one Opus agent and the spec-conformance verifier where aesthetic critique earns the premium. `ux-researcher` and `psych-scanner` run on Sonnet at high effort. `frontend-implementer`, `backend-implementer`, `advisor`, `test-writer`, and `compound-documenter` run on Sonnet at medium effort, lean execution against good specs and plans. Premium reasoning is concentrated on planning and final quality rather than spread across every step.
- **Lean from-scratch spine with opt-in depth.** The five-phase from-scratch pipeline becomes a lean default (problem, audience, MVP, IA, prototype, dev) plus an opt-in depth menu (competitor analysis, user interviews, StoryBrand, story panels, business plan, and the bias, ethics, and journey audits). The mandatory-dispatch rule is gone: dispatch an agent only when work would flood the main context, iterate inline otherwise. The question floor drops to "ask only what you can't infer, batched, no more than four at a time," and the advisor is optional rather than a required checkpoint.
- **Enforcement moved from hooks to method.** Grounding, anti-drift, TDD discipline, and canonical-path rules are now written into the skills and agents that own them, not enforced by deny-hooks. The kept hooks are advisory or mechanical only: project-state injection, Playwright path hygiene, the two Figma-intake prompts, deliverable dependency tracking, the session summary, post-compact recovery, and sound notifications.

### Why a major bump

Removed and renamed commands, agents, and skills; a reorganized pipeline; the deletion of the hook and cost-mode systems; and a new spec-driven design layer with workflow wirings are breaking changes by the plugin's own versioning rules.
