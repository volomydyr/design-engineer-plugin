...
description: Reviews designs and implementations for accessibility compliance including contrast, touch targets, and screen reader support. Use when auditing a product for accessibility or when building new features that need to meet accessibility standards.
...
Save to `{deliverables_path}/reviews/accessibility-review.md`.
...
After the accessibility review, suggest running `ui-visual-review` for a broader visual audit, or `ui-design-system` to ensure accessibility constants (like minimum tap target size) are part of the design token system.
