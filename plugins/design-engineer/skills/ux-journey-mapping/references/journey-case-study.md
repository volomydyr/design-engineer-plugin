...
| 3 | Default browser prompt | Drop / Pit | Negative | Immediately after install, Brave asks the user to change their default browser – before the user has even tried the product. Combined with the perception of needing to manually import bookmarks, motivation drops sharply. |
...
The journey starts at a medium motivation level (curiosity), maintains through the download decision, then drops sharply at the default browser prompt (Pit). The import completion provides a Jump back up, and the homepage dashboard delivers the Peak. The journey ends on a positive note with the first browsing session.
...
**Why it works**: When users cross a threshold of commitment, acknowledging that moment builds motivation. It signals "you are making progress" and primes the user for the next steps. An unacknowledged transition feels like the product does not care about the user's decision.
...
1. The user was asked to change their default browser before they could even try Brave – the commitment was too high for the motivation level at that moment
...
**Why it works**: Hyperbolic Discounting tells us that people prefer smaller, immediate rewards. By showing the dashboard first (even with initial data), you deliver an immediate reward that builds motivation before asking the user to invest effort in setup steps. The user thinks "this is what I am working toward" rather than "when will this setup end?"
...
The download decision is a significant milestone, but in many products, milestones like this go unacknowledged. Explicitly marking the Transition builds motivation and makes the user feel that the product respects their decision.
...
1. Map what is really happening to your customers right now (not the ideal scenario from your Story Panel)
...
4. Compare the current journey with the ideal one from your Story Panel to identify the biggest gaps
