...
2. It runs the context-analyzer agent as a sub-agent
3. The context analyzer asks you clarifying questions
4. You provide detailed answers
5. The main conversation creates a detailed implementation plan
6. It pauses for your manual approval
7. After approval, it runs the backend-implementer agent
8. Then runs the frontend-implementer agent
9. Finally runs the design-system-auditor agent
10. Results from all agents flow back to the main conversation
...
With sub-agents, each one uses its own set of tokens. One analyzes your project, another creates the plan, a third does the development. They all work in separate "chats" with separate limits, but results come back to the main conversation. This way you lose fewer tokens in the main conversation because all the heavy work happens elsewhere.
...
The approval step between planning and implementation is critical. After the context analyzer runs and you answer its questions, the main conversation creates a detailed implementation plan and saves it as a file.
