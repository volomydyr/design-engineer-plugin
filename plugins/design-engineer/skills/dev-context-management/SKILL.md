...
After context management is set up, suggest `dev-github-workflow` for version control (another context preservation mechanism), or `dev-agent-pipeline` to set up sub-agents that help preserve the main conversation's token budget.
