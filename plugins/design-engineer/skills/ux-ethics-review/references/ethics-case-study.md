# Ethics Case Study: Clubhouse Notifications Redesign
...
The notifications violated the "Value our attention" humane principle:

- **Volume**: 20+ notifications per day regardless of user engagement
- **Relevance**: Many notifications were not aligned with the user's actual interests
- **No adaptation**: When users stopped opening notifications, the system did not reduce frequency
- **Engagement-driven**: Notifications were timed for maximum opens, not user convenience
- **No user control**: Limited options for filtering by type or reducing frequency

### Running the ethics tests
...
**Notification preference screen**: When the system detects that a user has not engaged with notifications (e.g., ignored the last 20), instead of sending more, show a one-time preference screen that lets the user:

- Choose which types of content trigger notifications (topics, specific people, scheduled rooms)
- Set notification frequency (immediate, daily digest, weekly summary)
- Define quiet hours
- Opt for "highlights only" mode that surfaces only the most relevant content

**Key insight**: Allowing people to change their preferences to fit their needs is one of the best ways to re-engage them into using your app in a healthy manner.

## Lessons for Notification Design

### 1. Adaptation over persistence

If notifications are being ignored, the answer is never "send more." The answer is "send better" or "send fewer." Systems should learn from non-engagement and adapt.

### 2. User control builds trust

Giving users granular control over notifications seems counterintuitive from a growth perspective. But users who control their notifications are more likely to keep them on and engage with the ones they receive.

### 3. Quality over quantity

One highly relevant notification is worth more than 20 irrelevant ones. Relevance is measured by user action (opened, engaged with), not by sends.
...
Technology should be a helpful companion, not a needy acquaintance who texts you 20 times a day. Every notification should pass the test: "Would I appreciate receiving this right now?"

## Applying This to Your Product

For any notification or engagement system, ask:

1. Is each notification aligned with the user's goals, or with business engagement metrics?
