...
3. Share the Figma design via MCP and let AI apply the correct styles without touching the functionality
...
### Figma Console MCP (Unofficial)

**What it does:** Unlike the official Figma MCP that only reads design data, Figma Console can actually perform actions in Figma. It can turn raw frames into components with tokens and styles based on your prompt.

**When to install:** After you have explored the official Figma MCP and want more advanced design-to-code workflows. Particularly useful for design system work.

**Difficulty:** Trickier to set up than the official MCP, especially if you are new to Claude Code. But once learned, it is incredibly powerful for managing your design system programmatically.
...
The context-analyzer agent should be configured to use Context7 for documentation lookup. The frontend-implementer agent should reference Figma MCP for design data. Include these references in the agent templates.

### In Prompts

You can reference MCPs directly in prompts:
- "Use Context7 to look up the current approach for [topic]"
- "Use Figma MCP to analyze this design: [Figma link]"
