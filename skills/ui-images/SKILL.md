...
- `documents/design/design/references/references.md` (if exists) — visual style direction
- `documents/design/foundation/` — brand voice, target audience
- `documents/design/design/story-panels/` (if exists) — character/scene references
...
Save initial draft to `documents/design/design/images/manifest.md`. Create the folder if it doesn't exist.
...
2. **If Playwright CLI is available** (`compatibility: playwright-cli` was met):
   ...
   - Save the chosen image URL or download to `documents/design/design/images/stock/[image-id]-[short-desc].jpg`.
...
4. **Save each prompt** to `documents/design/design/images/prompts/[image-id]-[short-desc].md`. One file per image. The user can copy-paste the file content into their generator.

5. **Tell the user where to put the generated output**: `documents/design/design/images/generated/[image-id]-[short-desc].png` (or .webp/.jpg). Naming convention matches the prompt file.

## Step 5: Save manifest, lay out folders

Final state of `documents/design/design/images/`:

```
documents/design/design/images/
