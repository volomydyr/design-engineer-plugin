...
- The environment summary already produced by `skills/meta-setup/scripts/detect-environment.sh` (run during `/design-engineer:launch`).
...
- **Without a project structure.** If `.design-engineer-plugin/config.yaml` doesn't exist, route the user to `/design-engineer:launch` first.
