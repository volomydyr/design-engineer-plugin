...
When "HTML prototype" is selected, read the HTML file at the path the user provides (default: `{deliverables_path}/prototype/prototype.html`). Run the full accessibility checklist with particular attention to contrast ratios, touch/click target sizes, keyboard navigation, screen reader support (semantic HTML, ARIA labels), and heading hierarchy.
