...
If "Continue" or "Jump": suggest running `/product:design` to resume the pipeline.
If "Browse": show the full capability list inline (see below), then suggest relevant `/product:` commands.
...
If "Start": suggest running `/product:design`.
If "Browse": show the full capability list inline (see below), then suggest relevant `/product:` commands.
If "Reconfigure": proceed to Step 2.
...
# Generated by /product:launch on {current_date}
...
options:
  ...
  - label: "No"
    description: "Skip – re-run /product:launch later to install"
```
...
- If user picks "No, mute them" or "Keep muted": run `rm -f ~/.claude/de-sound-enabled` (idempotent; no error if absent). Confirm: "Sounds muted. Toggle anytime with /product:mute-unmute-sound."
...
Next step: Run /product:design to start designing your product.
Tip: Run /product:help anytime to see all available commands and capabilities.
```
...
4. Run `/product:launch` again – the script is fail-open and will skip unavailable checks
...
3. Delete any corrupted `.design-engineer-plugin/config.yaml` and re-run `/product:launch`
